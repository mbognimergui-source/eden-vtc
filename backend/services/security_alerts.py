...
from models.security_alerts import Security_alerts

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Security_alertsService:
    """Service layer for Security_alerts operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Security_alerts]:
        """Create a new security_alerts"""
        try:
            obj = Security_alerts(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created security_alerts with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating security_alerts: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Security_alerts]:
        """Get security_alerts by ID"""
        try:
            query = select(Security_alerts).where(Security_alerts.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching security_alerts {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of security_alertss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Security_alerts, field):
                        column = getattr(Security_alerts, field)
            ...
            stmt = select(Security_alerts, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Security_alerts, field_name):
                        stmt = stmt.order_by(getattr(Security_alerts, field_name).desc())
                else:
                    if hasattr(Security_alerts, sort):
                        stmt = stmt.order_by(getattr(Security_alerts, sort))
            else:
                stmt = stmt.order_by(Security_alerts.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Security_alerts)
                for condition in conditions:
            ...
            logger.error(f"Error fetching security_alerts list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Security_alerts]:
        """Update security_alerts"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Security_alerts {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated security_alerts {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating security_alerts {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Security_alerts {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted security_alerts {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting security_alerts {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Security_alerts]:
        """Get security_alerts by any field"""
        try:
            if not hasattr(Security_alerts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Security_alerts")
            column = getattr(Security_alerts, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Security_alerts).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching security_alerts by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Security_alerts]:
        """Get list of security_alertss filtered by field"""
        try:
            if not hasattr(Security_alerts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Security_alerts")
            column = getattr(Security_alerts, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Security_alerts.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching security_alertss by {field_name}: {str(e)}")
            raise
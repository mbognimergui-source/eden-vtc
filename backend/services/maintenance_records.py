...
from models.maintenance_records import Maintenance_records

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Maintenance_recordsService:
    """Service layer for Maintenance_records operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Maintenance_records]:
        """Create a new maintenance_records"""
        try:
            obj = Maintenance_records(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created maintenance_records with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating maintenance_records: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Maintenance_records]:
        """Get maintenance_records by ID"""
        try:
            query = select(Maintenance_records).where(Maintenance_records.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching maintenance_records {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of maintenance_recordss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Maintenance_records, field):
                        column = getattr(Maintenance_records, field)
            ...
            stmt = select(Maintenance_records, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Maintenance_records, field_name):
                        stmt = stmt.order_by(getattr(Maintenance_records, field_name).desc())
                else:
                    if hasattr(Maintenance_records, sort):
                        stmt = stmt.order_by(getattr(Maintenance_records, sort))
            else:
                stmt = stmt.order_by(Maintenance_records.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Maintenance_records)
                for condition in conditions:
            ...
            logger.error(f"Error fetching maintenance_records list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Maintenance_records]:
        """Update maintenance_records"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Maintenance_records {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated maintenance_records {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating maintenance_records {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete maintenance_records"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Maintenance_records {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted maintenance_records {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting maintenance_records {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Maintenance_records]:
        """Get maintenance_records by any field"""
        try:
            if not hasattr(Maintenance_records, field_name):
                raise ValueError(f"Field {field_name} does not exist on Maintenance_records")
            column = getattr(Maintenance_records, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Maintenance_records).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching maintenance_records by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Maintenance_records]:
        """Get list of maintenance_recordss filtered by field"""
        try:
            if not hasattr(Maintenance_records, field_name):
                raise ValueError(f"Field {field_name} does not exist on Maintenance_records")
            column = getattr(Maintenance_records, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Maintenance_records.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching maintenance_recordss by {field_name}: {str(e)}")
            raise
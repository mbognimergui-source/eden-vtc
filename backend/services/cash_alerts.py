...
# ------------------ Service Layer ------------------
class Cash_alertsService:
    """Service layer for Cash_alerts operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Cash_alerts]:
        """Create a new cash_alerts"""
        try:
            obj = Cash_alerts(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created cash_alerts with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating cash_alerts: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Cash_alerts]:
        """Get cash_alerts by ID"""
        try:
            query = select(Cash_alerts).where(Cash_alerts.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching cash_alerts {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of cash_alertss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Cash_alerts, field):
                        column = getattr(Cash_alerts, field)
            ...
            stmt = select(Cash_alerts, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Cash_alerts, field_name):
                        stmt = stmt.order_by(getattr(Cash_alerts, field_name).desc())
                else:
                    if hasattr(Cash_alerts, sort):
                        stmt = stmt.order_by(getattr(Cash_alerts, sort))
            else:
                stmt = stmt.order_by(Cash_alerts.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Cash_alerts)
                for condition in conditions:
            ...
            logger.error(f"Error fetching cash_alerts list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Cash_alerts]:
        """Update cash_alerts"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Cash_alerts {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated cash_alerts {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating cash_alerts {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Cash_alerts {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted cash_alerts {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting cash_alerts {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Cash_alerts]:
        """Get cash_alerts by any field"""
        try:
            if not hasattr(Cash_alerts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Cash_alerts")
            column = getattr(Cash_alerts, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Cash_alerts).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching cash_alerts by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Cash_alerts]:
        """Get list of cash_alertss filtered by field"""
        try:
            if not hasattr(Cash_alerts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Cash_alerts")
            column = getattr(Cash_alerts, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Cash_alerts.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching cash_alertss by {field_name}: {str(e)}")
            raise
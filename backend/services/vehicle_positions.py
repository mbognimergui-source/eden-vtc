...
# ------------------ Service Layer ------------------
class Vehicle_positionsService:
    """Service layer for Vehicle_positions operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Vehicle_positions]:
        """Create a new vehicle_positions"""
        try:
            obj = Vehicle_positions(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created vehicle_positions with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating vehicle_positions: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Vehicle_positions]:
        """Get vehicle_positions by ID"""
        try:
            query = select(Vehicle_positions).where(Vehicle_positions.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching vehicle_positions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of vehicle_positionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Vehicle_positions, field):
                        column = getattr(Vehicle_positions, field)
            ...
            stmt = select(Vehicle_positions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Vehicle_positions, field_name):
                        stmt = stmt.order_by(getattr(Vehicle_positions, field_name).desc())
                else:
                    if hasattr(Vehicle_positions, sort):
                        stmt = stmt.order_by(getattr(Vehicle_positions, sort))
            else:
                stmt = stmt.order_by(Vehicle_positions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Vehicle_positions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching vehicle_positions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Vehicle_positions]:
        """Update vehicle_positions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Vehicle_positions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated vehicle_positions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating vehicle_positions {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Vehicle_positions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted vehicle_positions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting vehicle_positions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Vehicle_positions]:
        """Get vehicle_positions by any field"""
        try:
            if not hasattr(Vehicle_positions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Vehicle_positions")
            column = getattr(Vehicle_positions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Vehicle_positions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching vehicle_positions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Vehicle_positions]:
        """Get list of vehicle_positionss filtered by field"""
        try:
            if not hasattr(Vehicle_positions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Vehicle_positions")
            column = getattr(Vehicle_positions, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Vehicle_positions.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching vehicle_positionss by {field_name}: {str(e)}")
            raise
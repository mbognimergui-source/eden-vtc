...
class User_rolesService:
    ...
    async def get_list(
        ...
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(User_roles.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(User_roles, field):
                        column = getattr(User_roles, field)
            ...
            stmt = select(User_roles, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(User_roles, field_name):
                        stmt = stmt.order_by(getattr(User_roles, field_name).desc())
                else:
                    if hasattr(User_roles, sort):
                        stmt = stmt.order_by(getattr(User_roles, sort))
            else:
                stmt = stmt.order_by(User_roles.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(User_roles)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[User_roles]:
        ...
        try:
            ...
            column = getattr(User_roles, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(User_roles).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:

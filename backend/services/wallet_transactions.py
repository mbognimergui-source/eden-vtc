...
class Wallet_transactionsService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Wallet_transactions]:
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Wallet_transactions]:
        """Get wallet_transactions by ID"""
        try:
    ...
    async def get_list(
        ...
        """Get paginated list of wallet_transactionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Wallet_transactions, field):
                        column = getattr(Wallet_transactions, field)
            ...
            stmt = select(Wallet_transactions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Wallet_transactions, field_name):
                        stmt = stmt.order_by(getattr(Wallet_transactions, field_name).desc())
                else:
                    if hasattr(Wallet_transactions, sort):
                        stmt = stmt.order_by(getattr(Wallet_transactions, sort))
            else:
                stmt = stmt.order_by(Wallet_transactions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Wallet_transactions)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Wallet_transactions]:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Wallet_transactions]:
        ...
        try:
            ...
            column = getattr(Wallet_transactions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Wallet_transactions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:

...
class VehiclesService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Vehicles]:
        ...
        try:
            ...
            logger.info(f"Created vehicles with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating vehicles: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Vehicles]:
        """Get vehicles by ID"""
        try:
            query = select(Vehicles).where(Vehicles.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching vehicles {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of vehicless"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Vehicles, field):
                        column = getattr(Vehicles, field)
            ...
            stmt = select(Vehicles, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Vehicles, field_name):
                        stmt = stmt.order_by(getattr(Vehicles, field_name).desc())
                else:
                    if hasattr(Vehicles, sort):
                        stmt = stmt.order_by(getattr(Vehicles, sort))
            else:
                stmt = stmt.order_by(Vehicles.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Vehicles)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Vehicles]:
        """Update vehicles"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Vehicles {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated vehicles {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating vehicles {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Vehicles {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted vehicles {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting vehicles {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Vehicles]:
        """Get vehicles by any field"""
        try:
            if not hasattr(Vehicles, field_name):
                raise ValueError(f"Field {field_name} does not exist on Vehicles")
            column = getattr(Vehicles, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Vehicles).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching vehicles by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Vehicles]:
        """Get list of vehicless filtered by field"""
        try:
            if not hasattr(Vehicles, field_name):
                raise ValueError(f"Field {field_name} does not exist on Vehicles")
            column = getattr(Vehicles, field_name)
            ...
            logger.error(f"Error fetching vehicless by {field_name}: {str(e)}")
            raise
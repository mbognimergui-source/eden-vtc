...
# ------------------ Service Layer ------------------
class Company_accountsService:
    """Service layer for Company_accounts operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Company_accounts]:
        """Create a new company_accounts"""
        try:
            obj = Company_accounts(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created company_accounts with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating company_accounts: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Company_accounts]:
        """Get company_accounts by ID"""
        try:
            query = select(Company_accounts).where(Company_accounts.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching company_accounts {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of company_accountss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Company_accounts, field):
                        column = getattr(Company_accounts, field)
            ...
            stmt = select(Company_accounts, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Company_accounts, field_name):
                        stmt = stmt.order_by(getattr(Company_accounts, field_name).desc())
                else:
                    if hasattr(Company_accounts, sort):
                        stmt = stmt.order_by(getattr(Company_accounts, sort))
            else:
                stmt = stmt.order_by(Company_accounts.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Company_accounts)
                for condition in conditions:
            ...
            logger.error(f"Error fetching company_accounts list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Company_accounts]:
        """Update company_accounts"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Company_accounts {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated company_accounts {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating company_accounts {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete company_accounts"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Company_accounts {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted company_accounts {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting company_accounts {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Company_accounts]:
        """Get company_accounts by any field"""
        try:
            if not hasattr(Company_accounts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Company_accounts")
            column = getattr(Company_accounts, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Company_accounts).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching company_accounts by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Company_accounts]:
        """Get list of company_accountss filtered by field"""
        try:
            if not hasattr(Company_accounts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Company_accounts")
            column = getattr(Company_accounts, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Company_accounts.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching company_accountss by {field_name}: {str(e)}")
            raise
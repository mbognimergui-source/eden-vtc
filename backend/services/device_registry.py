...
# ------------------ Service Layer ------------------
class Device_registryService:
    """Service layer for Device_registry operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Device_registry]:
        """Create a new device_registry"""
        try:
            obj = Device_registry(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created device_registry with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating device_registry: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Device_registry]:
        """Get device_registry by ID"""
        try:
            query = select(Device_registry).where(Device_registry.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching device_registry {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of device_registrys"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Device_registry, field):
                        column = getattr(Device_registry, field)
            ...
            stmt = select(Device_registry, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Device_registry, field_name):
                        stmt = stmt.order_by(getattr(Device_registry, field_name).desc())
                else:
                    if hasattr(Device_registry, sort):
                        stmt = stmt.order_by(getattr(Device_registry, sort))
            else:
                stmt = stmt.order_by(Device_registry.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Device_registry)
                for condition in conditions:
            ...
            logger.error(f"Error fetching device_registry list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Device_registry]:
        """Update device_registry"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Device_registry {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated device_registry {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating device_registry {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete device_registry"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Device_registry {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted device_registry {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting device_registry {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Device_registry]:
        """Get device_registry by any field"""
        try:
            if not hasattr(Device_registry, field_name):
                raise ValueError(f"Field {field_name} does not exist on Device_registry")
            column = getattr(Device_registry, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Device_registry).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching device_registry by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Device_registry]:
        """Get list of device_registrys filtered by field"""
        try:
            if not hasattr(Device_registry, field_name):
                raise ValueError(f"Field {field_name} does not exist on Device_registry")
            column = getattr(Device_registry, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Device_registry)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Device_registry.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching device_registrys by {field_name}: {str(e)}")
            raise
...
class FeedbacksService:
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Feedbacks]:
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for feedbacks {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Feedbacks]:
        """Get feedbacks by ID (user can only see their own records)"""
        try:
            query = select(Feedbacks).where(Feedbacks.id == obj_id)
            if user_id:
                query = query.where(Feedbacks.user_id == user_id)
    ...
    async def get_list(
        ...
        """Get paginated list of feedbackss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Feedbacks.user_id == user_id)
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Feedbacks]:
        """Update feedbacks (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
    ...
    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete feedbacks (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:

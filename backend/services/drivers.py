...
class DriversService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Drivers]:
        ...
        try:
            ...
            logger.info(f"Created drivers with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating drivers: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Drivers]:
        """Get drivers by ID"""
        try:
            query = select(Drivers).where(Drivers.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching drivers {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of driverss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Drivers, field):
                        column = getattr(Drivers, field)
            ...
            stmt = select(Drivers, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Drivers, field_name):
                        stmt = stmt.order_by(getattr(Drivers, field_name).desc())
                else:
                    if hasattr(Drivers, sort):
                        stmt = stmt.order_by(getattr(Drivers, sort))
            else:
                stmt = stmt.order_by(Drivers.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Drivers)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Drivers]:
        """Update drivers"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Drivers {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated drivers {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating drivers {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Drivers {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted drivers {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting drivers {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Drivers]:
        """Get drivers by any field"""
        try:
            if not hasattr(Drivers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Drivers")
            column = getattr(Drivers, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Drivers).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching drivers by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Drivers]:
        """Get list of driverss filtered by field"""
        try:
            if not hasattr(Drivers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Drivers")
            column = getattr(Drivers, field_name)
            ...
            logger.error(f"Error fetching driverss by {field_name}: {str(e)}")
            raise
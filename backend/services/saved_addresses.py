...
from models.saved_addresses import Saved_addresses

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Saved_addressesService:
    """Service layer for Saved_addresses operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Saved_addresses]:
        """Create a new saved_addresses"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Saved_addresses(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created saved_addresses with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating saved_addresses: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for saved_addresses {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Saved_addresses]:
        """Get saved_addresses by ID (user can only see their own records)"""
        try:
            query = select(Saved_addresses).where(Saved_addresses.id == obj_id)
            if user_id:
                query = query.where(Saved_addresses.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching saved_addresses {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of saved_addressess (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Saved_addresses.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Saved_addresses, field):
                        column = getattr(Saved_addresses, field)
            ...
            stmt = select(Saved_addresses, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Saved_addresses, field_name):
                        stmt = stmt.order_by(getattr(Saved_addresses, field_name).desc())
                else:
                    if hasattr(Saved_addresses, sort):
                        stmt = stmt.order_by(getattr(Saved_addresses, sort))
            else:
                stmt = stmt.order_by(Saved_addresses.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Saved_addresses)
                for condition in conditions:
            ...
            logger.error(f"Error fetching saved_addresses list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Saved_addresses]:
        """Update saved_addresses (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Saved_addresses {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated saved_addresses {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating saved_addresses {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete saved_addresses (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Saved_addresses {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted saved_addresses {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting saved_addresses {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Saved_addresses]:
        """Get saved_addresses by any field"""
        try:
            if not hasattr(Saved_addresses, field_name):
                raise ValueError(f"Field {field_name} does not exist on Saved_addresses")
            column = getattr(Saved_addresses, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Saved_addresses).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching saved_addresses by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Saved_addresses]:
        """Get list of saved_addressess filtered by field"""
        try:
            if not hasattr(Saved_addresses, field_name):
                raise ValueError(f"Field {field_name} does not exist on Saved_addresses")
            column = getattr(Saved_addresses, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Saved_addresses.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching saved_addressess by {field_name}: {str(e)}")
            raise
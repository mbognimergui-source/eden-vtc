...
class PassengersService:
    """Service layer for Passengers operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Passengers]:
        ...
        try:
            ...
            logger.info(f"Created passengers with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating passengers: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for passengers {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Passengers]:
        """Get passengers by ID (user can only see their own records)"""
        try:
            query = select(Passengers).where(Passengers.id == obj_id)
            if user_id:
                query = query.where(Passengers.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching passengers {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of passengerss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Passengers.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Passengers, field):
                        column = getattr(Passengers, field)
            ...
            stmt = select(Passengers, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Passengers, field_name):
                        stmt = stmt.order_by(getattr(Passengers, field_name).desc())
                else:
                    if hasattr(Passengers, sort):
                        stmt = stmt.order_by(getattr(Passengers, sort))
            else:
                stmt = stmt.order_by(Passengers.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Passengers)
                for condition in conditions:
            ...
            logger.error(f"Error fetching passengers list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Passengers]:
        """Update passengers (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Passengers {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated passengers {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating passengers {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete passengers (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Passengers {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted passengers {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting passengers {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Passengers]:
        """Get passengers by any field"""
        try:
            if not hasattr(Passengers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Passengers")
            column = getattr(Passengers, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Passengers).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching passengers by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Passengers]:
        """Get list of passengerss filtered by field"""
        try:
            if not hasattr(Passengers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Passengers")
            column = getattr(Passengers, field_name)
            ...
            logger.error(f"Error fetching passengerss by {field_name}: {str(e)}")
            raise
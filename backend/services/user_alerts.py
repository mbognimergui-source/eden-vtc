...
# ------------------ Service Layer ------------------
class User_alertsService:
    """Service layer for User_alerts operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[User_alerts]:
        """Create a new user_alerts"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = User_alerts(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created user_alerts with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating user_alerts: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for user_alerts {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[User_alerts]:
        """Get user_alerts by ID (user can only see their own records)"""
        try:
            query = select(User_alerts).where(User_alerts.id == obj_id)
            if user_id:
                query = query.where(User_alerts.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching user_alerts {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of user_alertss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(User_alerts.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(User_alerts, field):
                        column = getattr(User_alerts, field)
            ...
            stmt = select(User_alerts, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(User_alerts, field_name):
                        stmt = stmt.order_by(getattr(User_alerts, field_name).desc())
                else:
                    if hasattr(User_alerts, sort):
                        stmt = stmt.order_by(getattr(User_alerts, sort))
            else:
                stmt = stmt.order_by(User_alerts.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(User_alerts)
                for condition in conditions:
            ...
            logger.error(f"Error fetching user_alerts list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[User_alerts]:
        """Update user_alerts (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"User_alerts {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated user_alerts {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating user_alerts {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete user_alerts (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"User_alerts {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted user_alerts {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting user_alerts {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[User_alerts]:
        """Get user_alerts by any field"""
        try:
            if not hasattr(User_alerts, field_name):
                raise ValueError(f"Field {field_name} does not exist on User_alerts")
            column = getattr(User_alerts, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(User_alerts).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching user_alerts by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[User_alerts]:
        """Get list of user_alertss filtered by field"""
        try:
            if not hasattr(User_alerts, field_name):
                raise ValueError(f"Field {field_name} does not exist on User_alerts")
            column = getattr(User_alerts, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(User_alerts.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching user_alertss by {field_name}: {str(e)}")
            raise
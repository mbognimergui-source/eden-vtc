...
# ------------------ Service Layer ------------------
class Country_tariffsService:
    """Service layer for Country_tariffs operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Country_tariffs]:
        """Create a new country_tariffs"""
        try:
            obj = Country_tariffs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created country_tariffs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating country_tariffs: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Country_tariffs]:
        """Get country_tariffs by ID"""
        try:
            query = select(Country_tariffs).where(Country_tariffs.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching country_tariffs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of country_tariffss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Country_tariffs, field):
                        column = getattr(Country_tariffs, field)
            ...
            stmt = select(Country_tariffs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Country_tariffs, field_name):
                        stmt = stmt.order_by(getattr(Country_tariffs, field_name).desc())
                else:
                    if hasattr(Country_tariffs, sort):
                        stmt = stmt.order_by(getattr(Country_tariffs, sort))
            else:
                stmt = stmt.order_by(Country_tariffs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Country_tariffs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching country_tariffs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Country_tariffs]:
        """Update country_tariffs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Country_tariffs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated country_tariffs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating country_tariffs {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete country_tariffs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Country_tariffs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted country_tariffs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting country_tariffs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Country_tariffs]:
        """Get country_tariffs by any field"""
        try:
            if not hasattr(Country_tariffs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Country_tariffs")
            column = getattr(Country_tariffs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Country_tariffs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching country_tariffs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Country_tariffs]:
        """Get list of country_tariffss filtered by field"""
        try:
            if not hasattr(Country_tariffs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Country_tariffs")
            column = getattr(Country_tariffs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Country_tariffs)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Country_tariffs.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching country_tariffss by {field_name}: {str(e)}")
            raise
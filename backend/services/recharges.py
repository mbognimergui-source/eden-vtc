...
from models.recharges import Recharges

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class RechargesService:
    """Service layer for Recharges operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Recharges]:
        ...
        try:
            ...
            logger.info(f"Created recharges with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating recharges: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Recharges]:
        """Get recharges by ID"""
        try:
            query = select(Recharges).where(Recharges.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching recharges {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of rechargess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Recharges, field):
                        column = getattr(Recharges, field)
            ...
            stmt = select(Recharges, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Recharges, field_name):
                        stmt = stmt.order_by(getattr(Recharges, field_name).desc())
                else:
                    if hasattr(Recharges, sort):
                        stmt = stmt.order_by(getattr(Recharges, sort))
            else:
                stmt = stmt.order_by(Recharges.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Recharges)
                for condition in conditions:
            ...
            logger.error(f"Error fetching recharges list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Recharges]:
        """Update recharges"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Recharges {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated recharges {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating recharges {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Recharges {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted recharges {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting recharges {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Recharges]:
        """Get recharges by any field"""
        try:
            if not hasattr(Recharges, field_name):
                raise ValueError(f"Field {field_name} does not exist on Recharges")
            column = getattr(Recharges, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Recharges).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching recharges by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Recharges]:
        """Get list of rechargess filtered by field"""
        try:
            if not hasattr(Recharges, field_name):
                raise ValueError(f"Field {field_name} does not exist on Recharges")
            column = getattr(Recharges, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Recharges.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching rechargess by {field_name}: {str(e)}")
            raise
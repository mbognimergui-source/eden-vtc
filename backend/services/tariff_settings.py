...
# ------------------ Service Layer ------------------
class Tariff_settingsService:
    """Service layer for Tariff_settings operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Tariff_settings]:
        """Create a new tariff_settings"""
        try:
            obj = Tariff_settings(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created tariff_settings with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating tariff_settings: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Tariff_settings]:
        """Get tariff_settings by ID"""
        try:
            query = select(Tariff_settings).where(Tariff_settings.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching tariff_settings {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of tariff_settingss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Tariff_settings, field):
                        column = getattr(Tariff_settings, field)
            ...
            stmt = select(Tariff_settings, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Tariff_settings, field_name):
                        stmt = stmt.order_by(getattr(Tariff_settings, field_name).desc())
                else:
                    if hasattr(Tariff_settings, sort):
                        stmt = stmt.order_by(getattr(Tariff_settings, sort))
            else:
                stmt = stmt.order_by(Tariff_settings.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Tariff_settings)
                for condition in conditions:
            ...
            logger.error(f"Error fetching tariff_settings list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Tariff_settings]:
        """Update tariff_settings"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Tariff_settings {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated tariff_settings {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating tariff_settings {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete tariff_settings"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Tariff_settings {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted tariff_settings {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting tariff_settings {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Tariff_settings]:
        """Get tariff_settings by any field"""
        try:
            if not hasattr(Tariff_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Tariff_settings")
            column = getattr(Tariff_settings, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Tariff_settings).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching tariff_settings by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Tariff_settings]:
        """Get list of tariff_settingss filtered by field"""
        try:
            if not hasattr(Tariff_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Tariff_settings")
            column = getattr(Tariff_settings, field_name)
            ...
            logger.error(f"Error fetching tariff_settingss by {field_name}: {str(e)}")
            raise
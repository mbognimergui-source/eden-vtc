...
# ------------------ Service Layer ------------------
class RidesService:
    """Service layer for Rides operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Rides]:
        """Create a new rides"""
        try:
            obj = Rides(**data)
            self.db.add(obj)
            ...
            logger.info(f"Created rides with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating rides: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Rides]:
        """Get rides by ID"""
        try:
            query = select(Rides).where(Rides.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching rides {obj_id}: {str(e)}")
            raise
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Rides, field):
                        column = getattr(Rides, field)
            ...
            stmt = select(Rides, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Rides, field_name):
                        stmt = stmt.order_by(getattr(Rides, field_name).desc())
                else:
                    if hasattr(Rides, sort):
                        stmt = stmt.order_by(getattr(Rides, sort))
            else:
                stmt = stmt.order_by(Rides.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Rides)
                for condition in conditions:
            ...
            logger.error(f"Error fetching rides list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Rides]:
        """Update rides"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Rides {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated rides {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating rides {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Rides {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted rides {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting rides {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Rides]:
        """Get rides by any field"""
        try:
            if not hasattr(Rides, field_name):
                raise ValueError(f"Field {field_name} does not exist on Rides")
            column = getattr(Rides, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Rides).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching rides by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Rides]:
        """Get list of ridess filtered by field"""
        try:
            if not hasattr(Rides, field_name):
                raise ValueError(f"Field {field_name} does not exist on Rides")
            column = getattr(Rides, field_name)
            ...
            logger.error(f"Error fetching ridess by {field_name}: {str(e)}")
            raise
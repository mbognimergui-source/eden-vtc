...
# ------------------ Service Layer ------------------
class Cash_register_transactionsService:
    """Service layer for Cash_register_transactions operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Cash_register_transactions]:
        """Create a new cash_register_transactions"""
        try:
            obj = Cash_register_transactions(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created cash_register_transactions with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating cash_register_transactions: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Cash_register_transactions]:
        """Get cash_register_transactions by ID"""
        try:
            query = select(Cash_register_transactions).where(Cash_register_transactions.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching cash_register_transactions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of cash_register_transactionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Cash_register_transactions, field):
                        column = getattr(Cash_register_transactions, field)
            ...
            stmt = select(Cash_register_transactions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Cash_register_transactions, field_name):
                        stmt = stmt.order_by(getattr(Cash_register_transactions, field_name).desc())
                else:
                    if hasattr(Cash_register_transactions, sort):
                        stmt = stmt.order_by(getattr(Cash_register_transactions, sort))
            else:
                stmt = stmt.order_by(Cash_register_transactions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Cash_register_transactions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching cash_register_transactions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Cash_register_transactions]:
        """Update cash_register_transactions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Cash_register_transactions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated cash_register_transactions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating cash_register_transactions {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete cash_register_transactions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Cash_register_transactions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted cash_register_transactions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting cash_register_transactions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Cash_register_transactions]:
        """Get cash_register_transactions by any field"""
        try:
            if not hasattr(Cash_register_transactions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Cash_register_transactions")
            column = getattr(Cash_register_transactions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Cash_register_transactions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching cash_register_transactions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Cash_register_transactions]:
        """Get list of cash_register_transactionss filtered by field"""
        try:
            if not hasattr(Cash_register_transactions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Cash_register_transactions")
            column = getattr(Cash_register_transactions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Cash_register_transactions)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Cash_register_transactions.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching cash_register_transactionss by {field_name}: {str(e)}")
            raise